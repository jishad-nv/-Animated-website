 ...
 */
function syncStatusToDb(productId, status, glbUrl, db) {
    if (!db) return;
    const id = parseInt(productId, 10);
    try {
        ...
        db.prepare(`
            ...
            WHERE product_id = ?
        `).run(status, glbUrl, enableVal, id);
    } catch (e) {
 ...
 */
function deleteProduct3DModel(productId, db) {
    const id = parseInt(productId, 10);
    const row = db.prepare('SELECT glb_url FROM products WHERE product_id = ?').get(id);
 ...
 */
function toggleProduct3D(productId, enable, db) {
    const id = parseInt(productId, 10);
    const val = enable ? 1 : 0;
    db.prepare(`
        UPDATE products SET enable_3d = ?, updated_at = CURRENT_TIMESTAMP WHERE product_id = ?
    `).run(val, id);
 ...
 */
function saveUploadedGlb(productId, glbBuffer, db) {
    const id = parseInt(productId, 10);
    ...
    const oldRow = db.prepare('SELECT glb_url FROM products WHERE product_id = ?').get(id);
